...
public class ResumeApiController {
    ...
    public ResponseEntity<ResumeAnalysis> analyzeResume(
        ...
        if (file == null || file.isEmpty()) {
            throw new InvalidInputException("Please upload a valid non-empty PDF file.");
        }

        try (InputStream is = file.getInputStream()) {
            String text = resumeParserService.extractTextFromPdf(is);
    ...
    public ResponseEntity<ResumeComparison> compareResumes(
        ...
        if (resumes.isEmpty()) {
            throw new InvalidInputException("No readable PDF resumes could be extracted from the uploaded files.");
        }
